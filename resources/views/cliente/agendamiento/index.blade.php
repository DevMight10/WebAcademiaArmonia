...
@php
    $totalMinutosDisponibles = $beneficiario->distribuciones()
...
<x-card>
    <form method="POST" action="{{ route('cliente.agendamiento.store') }}" id="form-agendamiento">
        @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    instrumentoSelect.addEventListener('change', async function() {
        ...
        try {
            const response = await fetch(`/cliente/agendamiento/instructores/${instrumentoId}`);
    ...
    function actualizarResumen() {
        ...
        if (instrumento && instructor && fecha && hora && duracion) {
            ...
            document.getElementById('resumen-fecha').textContent = new Date(fecha + 'T00:00:00').toLocaleDateString('es-ES');
