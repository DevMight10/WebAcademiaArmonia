...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Editar Instructor</h1>
            <p class="mt-1 text-sm text-gray-600">Actualiza los datos del instructor</p>
        </div>
    </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.instructores.update', $instructore) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text"
                           name="nombre"
                           id="nombre"
                           value="{{ old('nombre', explode(' ', $instructore->user->name)[0] ?? '') }}"
                ...
                <div>
                    ...
                    @php
                        $nameParts = explode(' ', $instructore->user->name);
                        $apellido = count($nameParts) > 1 ? implode(' ', array_slice($nameParts, 1)) : '';
                    @endphp
                    <input type="text"
                           name="apellido"
                           id="apellido"
                           value="{{ old('apellido', $apellido) }}"
                ...
                <div>
                    ...
                    <input type="text"
                           name="telefono"
                           id="telefono"
                           value="{{ old('telefono', $instructore->telefono) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="email"
                           name="email"
                           id="email"
                           value="{{ old('email', $instructore->user->email) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="categoria"
                        ...
                        @foreach($categorias as $cat)
                            <option value="{{ $cat->value }}"
                                    {{ old('categoria', $instructore->categoria) == $cat->value ? 'selected' : '' }}
                    ...
                    @error('categoria')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-sm text-gray-500">Factor de costo actual: {{ number_format($instructore->factor_costo, 2) }}x</p>
                </div>
                    ...
                    <div class="flex items-center h-[42px]">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox"
                                   ...
                                   id="estado"
                                   value="1"
                                   {{ old('estado', $instructore->estado) ? 'checked' : '' }}
                                   class="sr-only peer">
                    ...
                    @foreach($instrumentos as $instrumento)
                        <label class="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input type="checkbox"
                                   ...
                                   {{ in_array($instrumento->id, old('especialidades', $especialidadesActuales)) ? 'checked' : '' }}
